...
@NamedQueries({ 
	@NamedQuery(
			name = "AccountUser.all", 
			query = "SELECT a FROM HibernateAccountUser a"),
	@NamedQuery(
			name = "AccountUser.byAccountAndUser", 
			query = "SELECT a FROM HibernateAccountUser a WHERE a.accountId=:accountId AND a.userId=:userId") 
	})
public class HibernateAccountUser implements AccountUser, ApplicationAccountEntityProxy, ProxyHasSecondaryKey {

	private static final long serialVersionUID = 698788636930696L;
	
	private final String allQueryName = "AccountUser.all";
